...
class ProjectService {
  ...
  Future<void> deleteProject(String id) async {
    ...
    if (project != null) {
      await _deleteProjectMedia(project);
    }
  ...
  /// 删除项目相关的所有媒体文件
  Future<void> _deleteProjectMedia(Project project) async {
    print('🗑️ 删除项目媒体文件...');
    int deletedCount = 0;
    ...
    for (final message in project.messages) {
      ...
      if (message.mediaPath != null && message.mediaPath!.isNotEmpty) {
        await _deleteMediaFile(message.mediaPath!);
        deletedCount++;
      }

      // 删除缩略图
      if (message.thumbnailPath != null && message.thumbnailPath!.isNotEmpty) {
        await _deleteMediaFile(message.thumbnailPath!);
        deletedCount++;
      }
      ...
      if (message.mediaList != null) {
        for (final media in message.mediaList!) {
          ...
          if (media.thumbnailPath != null && media.thumbnailPath!.isNotEmpty) {
            await _deleteMediaFile(media.thumbnailPath!);
            deletedCount++;
          }
        }
      }
